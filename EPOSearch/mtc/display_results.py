...
fig = plt.figure()
fig.subplots_adjust(left=0.12, bottom=0.1, right=0.78, top=1)
# Baseline stem plot
...
markerline, stemlines, baseline = plt.stem(
    baseline_l, label="Baseline", use_line_collection=True, basefmt=" "
)
...
for mid, method in enumerate(methods):
    ...
    # print(f"stds={rl_std}")
    plt.plot(
        np.arange(m) + 0.1 * (mid - 1),
        rl_mean,
        c=colors[method],
        lw=0.5,
        marker=markers[method],
        ms=marker_szs[method],
        label=method,
    )
    plt.errorbar(
        np.arange(len(rl_mean)) + 0.1 * (mid - 1),
        rl_mean,
        elinewidth=0.9,
        yerr=rl_std,
        fmt=" ",
