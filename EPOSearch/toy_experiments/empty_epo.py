...
if __name__ == "__main__":
    ...
    for ovs in [pf, ls_left, ls_right]:
        ovs += shift
    label = "Pareto\nFront"
    plt.plot(pf[:, 0], pf[:, 1], lw=5, c="k", zorder=-2, alpha=0.4)  # label=label,
    plt.plot(ls_left[:, 0], ls_left[:, 1], lw=1.5, ls="-", c="k", zorder=-2, alpha=0.4)
    plt.plot(
        ls_right[:, 0], ls_right[:, 1], lw=1.5, ls="-", c="k", zorder=-2, alpha=0.4
    )
    ...
    for k, r in enumerate(rs):
        ...
        x0, res = epo_search(
            shifted_concave_fun_eval, r=r, x=x0, step_size=ss, max_iters=max_iters
        )
        ls = res["ls"]
        if k > 0:
            traj_label = ""
