...
for method in data:
    ...
    for m in data[method]:
        ...
        if method == "PMTL":
            mu_rl = df[r"$\mu_r(l)$"]
            df = df[mu_rl.between(mu_rl.quantile(0.1), mu_rl.quantile(0.9))]
        dfs.append(df)
...
fig.subplots_adjust(left=0.11, bottom=0.1, right=0.96, top=0.98)
g = sns.pointplot(
    x=r"$m$",
    y=r"$\mu_r(l)$",
    hue="Method",
...
for method in timings:
    ...
    for m in timings[method]:
        ...
        if method == "PMTL":
            ...
            df = df[tm.between(tm.quantile(0.1), tm.quantile(0.9))]
        dfs.append(df)
    method_df = pd.concat(dfs, ignore_index=True)
    method_df["Method"] = method
    method_dfs.append(method_df)
...
fig = plt.figure(figsize=(3, 2.7))
fig.subplots_adjust(left=0.16, bottom=0.1, right=0.96, top=0.98)
g = sns.pointplot(
    x=r"$m$",
