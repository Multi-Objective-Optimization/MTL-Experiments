    ...
    for method in methods:
        ...
        for k, r in enumerate(rs):
            ...
            x0 = (
                ...
                if method in ["MOOMTL", "LinScalar"]
                else x0
            )
            if method == "EPO":
                _, res = epo_search(
                    concave_fun_eval, r=r, x=x0, step_size=ss, max_iters=100
                )
            if method == "PMTL":
            ...
            if method == "LinScalar":
                _, res = linscalar(
                    concave_fun_eval, r=r, x=x0, step_size=ss, max_iters=mi
                )
            if method == "MOOMTL":
                _, res = moo_mtl_search(
                    concave_fun_eval, x=x0, step_size=0.2, max_iters=150
                )
