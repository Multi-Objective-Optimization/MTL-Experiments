...
def create_pf(side_nonpf=False):
    ...
    if side_nonpf:
        ps = np.linspace(-1.0 / np.sqrt(2), 1.0 / np.sqrt(2), 30, endpoint=True)
    else:
    ...
    if side_nonpf:
        s_left = np.linspace(-1.5 / np.sqrt(2), -1.0 / np.sqrt(2), 10, endpoint=True)
        fs_left = map_to_objective_space(s_left)

        s_right = np.linspace(1.0 / np.sqrt(2), 1.5 / np.sqrt(2), 10, endpoint=True)
...
def add_interval(ax, xdata, ydata, color="k", caps="  ", label="", side="both", lw=2):
    line = ax.add_line(mpl.lines.Line2D(xdata, ydata))
    line.set_label(label)
    line.set_color(color)
    line.set_linewidth(lw)
    anno_args = {"ha": "center", "va": "center", "size": 12, "color": line.get_color()}
    a = []
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    fig.subplots_adjust(left=0.025, bottom=0.12, right=0.975, top=0.975)
    ...
    add_interval(
        ...
        + r"$\theta^0 \succcurlyeq \mathbf{1}/\sqrt{n}$",
        side="right",
    )
