...
def get_concave_func_eval(n, m):
    shifts = np.random.uniform(-1.0 / n, 1.0 / n, (m, n))
...
def get_concave_func_eval_gpu(n, m):
    shifts = (2.0 / n) * torch.rand(m, n).cuda() - 1.0 / n
...
if __name__ == "__main__":
    ...
    data = {"EPO Search": {m: [] for m in ms}, "PMTL": {m: [] for m in ms}}
    timings = {"EPO Search": {m: [] for m in ms}, "PMTL": {m: [] for m in ms}}
    for m in ms:
        print(f"m={m}")
        ...
        for n in ns:
            ...
            r /= np.sum(r)
            t0 = time()
            _, res = epo_search(f_df, r=r, step_size=ss, n_dim=n, max_iters=1000)
            t1 = time()
            ...
            t0 = time()
            x, res = pareto_mtl_search(
                f_df_gpu, ref_vecs=r_invs, r=k, step_size=ss, n_dim=n, max_iters=100
            )
            t1 = time()
