...
if __name__ == "__main__":
    ...
    for method in methods:
        fig, ax = plt.subplots()
        fig.subplots_adjust(left=0.12, bottom=0.12, right=0.975, top=0.975)
        ...
        for k, r in enumerate(rs):
            if init_type == "hard":
                x0 = np.random.uniform(0.15 * (-1) ** (k), 0.5 * (-1) ** (k), n)
            else:
                x0 = np.random.uniform(-0.4, 0.4, n)
            l0, _ = concave_fun_eval(x0)
            label = (
                r"$l(\theta^0)$"
                if init_type == "hard" and k == 0 and method == "PMTL"
                else ""
            )
            plt.scatter([l0[0]], [l0[1]], c=colors[k], s=35, label=label, zorder=2)
            r_inv = 1.0 / r
            if init_type == "hard":
                ...
                ep_ray = (
                    r_inv * (r_inv @ np.array([0.9, 0.9])) / np.linalg.norm(r_inv) ** 2
                )

            ep_ray_line = np.stack([np.zeros(m), ep_ray])
            label = (
                r"$r^{-1}$ ray"
                if k == 0 and init_type == "easy" and method == "EPO"
                else ""
            )
            ax.plot(
                ep_ray_line[:, 0],
            ...
            if method == "EPO":
                _, res = epo_search(
                    concave_fun_eval, r=r, x=x0, step_size=ss_epo, max_iters=mi_epo
                )
                ls = res["ls"]
            else:
                _, res = pareto_mtl_search(
                    concave_fun_eval,
                    ref_vecs=pmtl_refs,
                    r=r_inv,
                    x=x0,
            ...
            label = (
                "PMTL Phase 2"
                if k == 0 and init_type == "easy" and method == "PMTL"
                else ""
            )
