...
for dataset in datasets:
    ...
    for method in ["EPO", "PMTL", "indiv", "LinScalar"]:
        ...
        data[dataset][method] = {"last_ls": last_ls, "last_acs": last_acs, "rs": rs}
        if method == "LinScalar":
...
for dataset in datasets:
    ...
    ax = brokenaxes(
        xlims=((-0.05, max2x + 0.05), (max1x - 0.1, max1x + 0.15)),
        ylims=((-0.05, max2y + 0.05), (max1y - 0.1, max1y + 0.1)),
        hspace=0.05,
        wspace=0.05,
        fig=fig,
    )
    ...
    colors = []
    for i, (r, l) in enumerate(zip(data[dataset]["rs"], data[dataset]["rlens"])):
