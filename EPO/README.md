...
\tag{12}
$$
trong đó các trọng số $a_j$ là các độ lệch (deviation) tương đối tương ứng so với tính phi đồng nhất tổng thể $\mu_r(l)$, mà chúng tôi gọi là điều chỉnh (adjustments):
...
**Chứng minh:**
- Phép khai triển Taylor bậc nhất của hàm mất mát đa mục tiêu $l(\theta - \eta d)$ được sử dụng để xác định cận dưới của $\mu_r(l(\theta)) - \mu_r(l(\theta - \eta d))$. Sau đó chứng minh rằng cận dưới này dương nếu $d^T d_{bal} > 0$.

Lưu ý rằng các adjustment $a_j$ là không âm đối với một số mục tiêu và âm đối với phần còn lại, tức là trừ khi tất cả $r_j l_j$ đều bằng nhau, sẽ luôn có một số mục tiêu có các số hạng gradient trong $d_{bal}$ là âm. 

Đặt $J = \{j \mid g_j^T d_{bal} > 0\}$ là tập chỉ số của tất cả gradient nằm trong nửa mặt phẳng dương của $d_{bal}$, và $\bar{J} = [m] - J$ là tập chỉ số của các gradient trong nửa mặt phẳng âm của $d_{bal}$. Nếu chúng tôi sử dụng $d_{bal}$ để di chuyển đến nghiệm mới $\theta_\eta^{t+1} = \theta^t - \eta d_{bal}$, thì với cỡ bước đủ nhỏ $\eta_0 > 0$:
...
Ngoài việc tạo điều kiện cho một hướng cân bằng trong không gian nghiệm $\mathbb{R}^n$, các $a$ trong (13) còn có những tính chất thú vị trong không gian mục tiêu $\mathbb{R}^m$. Nếu ta xây dựng $m$ vectơ trong $\mathbb{R}^m$ bằng cách sử dụng các gradient như sau:
...
thì các tập chỉ số $J$ và $\bar{J}$ có thể được viết lại thành:

$$
J = \{j \mid a^T c_j > 0\}
\quad \text{và} \quad
\bar{J} = \{j \mid a^T c_j \leq 0\}.
\tag{17}
$$
...
$$
C = G^T G \in \mathbb{R}^{m \times m}
$$

với các cột là $c_j$ là một ma trận đối xứng. Một tính chất đáng chú ý khác của $a$ là mối quan hệ của nó với vectơ mất mát $l$:

**Claim 1:**
Vectơ adjustment $a$ vuông góc với $l$ trong không gian mục tiêu $\mathbb{R}^m$.
...
Từ \cite{Desideri2012}, chúng tôi biết rằng hướng giảm $d \in \mathcal{CH}_{\theta^t}$, bao lồi của các gradient tại $\theta^t$, sẽ hướng về phía biên Pareto. Từ (4) và (5), $d = G\beta$, trong đó $\beta \in \mathcal{S}^m$. 

Từ **Theorem 1**, chúng tôi biết rằng di chuyển ngược 1 hướng sẽ tạo 1 góc dương với $d_{bal} = Ga$, sẽ cải thiện tính đồng nhất. Kết hợp hai yêu cầu, chúng tôi muốn tìm hướng trong $\mathcal{CH}_{\theta^t}$ có góc cực đại với $d_{bal}$, tức là chúng tôi tối đa hóa:
...
Kết hợp (19) và (20) cho hàm mục tiêu trong bài toán LP (24a) với $\mathbb{1}_{\mu_r^t}$ là scalar indicator cho $\mu_r(l^t)$ khác không.

Trong chế độ giảm thuần túy, tức là khi tối đa hóa (20), chúng tôi chỉ cần ràng buộc hướng giảm:
...
Khi tối đa hóa (19), góc mà $d$ tạo với một số gradient $g_j$ cần phải âm, để cho phép bước tăng cho các mục tiêu có giá trị thấp $r_j l_j^t$ và bước giảm cho các mục tiêu có giá trị cao. Từ (15), chúng tôi biết rằng bước tăng có thể xảy ra cho tất cả các mục tiêu trong $\bar{J}$ nếu $d_{bal} \notin \mathcal{CH}_{\theta^t}$. Khi $d_{bal} \notin \mathcal{CH}_{\theta^t}$, chúng tôi có thể cho phép $d$ yêu cầu có góc âm với các gradient của các mục tiêu trong $\bar{J}$. Vì $a^T c_j \leq 0$, chúng tôi đạt được điều này bằng cách sửa đổi (21) thành $\beta^T c_j \geq a^T c_j \mathbb{1}_J$, $\forall j \in \bar{J}$.

Khi $d_{bal}$ không tạo góc dương với bất kỳ gradient nào, tức là $J$ rỗng và $\bar{J} = [m]$, chúng tôi không nên cho phép góc âm giữa $d$ và các gradient, để tránh tăng tất cả các giá trị mục tiêu đồng thời. Điều này có thể được đảm bảo bằng cách sử dụng scalar indicator $\mathbb{1}_J$ cho tập không rỗng $J$ và thêm ràng buộc:
