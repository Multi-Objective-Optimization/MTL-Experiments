    ...
    with open(path, 'rb') as f:
        ...
        for left in range(length):
            ...
            for j, right in enumerate(chosen_ones):
                ...
                multi_data_im = np.array(Image.fromarray(new_im.astype(np.uint8)).resize((28, 28), Image.NEAREST))
                multi_data[left*1 + j,:,:] = multi_data_im
        return torch.from_numpy(parsed.copy()).view(length, num_rows, num_cols), torch.from_numpy(multi_data).view(length, num_rows, num_cols), extension
