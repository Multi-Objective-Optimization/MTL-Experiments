...
class CITYSCAPES(data.Dataset):
    ...
    def __init__(self, root, split=["train"], is_transform=False,
                 img_size=(512, 1024), augmentations=None, depth_mean_path='depth_mean.npy'):
        self.root = root
        self.split = split
    ...
    def transform(self, img, lbl, ins_y, ins_x, depth):
        ...
        lbl = np.array(Image.fromarray(lbl.astype(np.float32)).resize((w8, h8), Image.NEAREST)).astype(int)

        ins_y = np.array(Image.fromarray(ins_y.astype(np.float32)).resize((w8, h8), Image.NEAREST))
        ins_x = np.array(Image.fromarray(ins_x.astype(np.float32)).resize((w8, h8), Image.NEAREST))
        depth = np.array(Image.fromarray(depth.astype(np.float32)).resize((w8, h8), Image.NEAREST))
