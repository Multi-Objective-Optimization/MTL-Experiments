...
class RunningMetric(object):
    def __init__(self, metric_type, n_classes=None, ignore_index=250):
    ...
    def _fast_hist(self, pred, gt):
        ...
        hist = np.bincount(
            self._n_classes * gt[mask].astype(int) + pred[mask],
    ...
    def update(self, pred, gt):
        if self._metric_type == "ACC":
            predictions = pred.detach().max(1, keepdim=True)[1]
            self.accuracy += (
                predictions.eq(gt.detach().view_as(predictions)).cpu().sum()
            )
            self.num_updates += predictions.shape[0]

        if self._metric_type == "L1":
            _gt = gt.detach().cpu().numpy()
            _pred = pred.detach().cpu().numpy()
            mask = _gt != self._ignore_index
            if np.sum(mask) < 1:
                return
            self.l1 += np.sum(np.abs(_gt[mask] - _pred[mask]))
            self.num_updates += np.sum(mask)

        if self._metric_type == "IOU":
            _pred = pred.detach().max(1)[1].cpu().numpy()
            _gt = gt.detach().cpu().numpy()
    ...
    def get_result(self):
        ...
        if self._metric_type == "IOU":
            if self.confusion_matrix.sum() == 0:
                return {"micro_acc": 0.0, "macro_acc": 0.0, "mIOU": 0.0}
...
def get_metrics(cfg):
    ...
    if "cityscapes" in cfg["dataset"]:
        n_classes = cfg.get("num_classes", 19)
        ignore_index = cfg.get("ignore_index", 250)
        if "S" in cfg["tasks"]:
            met["S"] = RunningMetric(metric_type="IOU", n_classes=n_classes)
        if "I" in cfg["tasks"]:
            met["I"] = RunningMetric(metric_type="L1", ignore_index=ignore_index)
        if "D" in cfg["tasks"]:
            met["D"] = RunningMetric(metric_type="L1", ignore_index=0)
